...
**3. Today's move (hero callout).** Violet-tinted card, full-width, sits above the active deals table. *Single rotating prescriptive line: "what's the most leveraged move I could make today across my pipeline?"* Reads like a coach, not a CRM.

Daily-rotating examples:
- *"Maria's proposal has been open 14 days. Today's move: nudge her with the deck."*
- *"Fenton retainer renews in 38 days. Today's move: schedule the Q3 strategy call."*
- *"Three new referrals from Sharisse this week — none touched. Today's move: send a short intro to all three."*

Each card surfaces:
- A single sentence framing the move
- A primary action button (e.g. *"Open Maria's record"* or *"Draft the message"*)
- A subtle "why this?" tooltip for context (post-v1 — designer should slot but not build)

The selection logic (post-v1) ranks across: hot opportunities about to close, warm leads going cold, renewals approaching, referrals untouched, proposals awaiting response. *Not* stuck deals as a category — single-operator pipelines don't die from stuck deals, they die from missed best-next-moves.
...
**4a. Stuck deals banner (secondary).** Above the active deals table, a thin amber-tinted strip — *only when the count is non-zero* — that says *"N deals untouched 30+ days · review."* Clicking filters the table. Demoted from hero status because it's diagnostic, not prescriptive — useful, but not the question Chris opens this page to answer. The hero is "what's the move"; stuck deals is "what could fall through the cracks."
...
## Future hero candidate (v2, post-launch idea)

A *"Capacity reality check"* hero — sits on the Today page (not Pipeline), tells Chris whether his calendar has room to take on new client work or whether he's in protect-mode. Combines calendar load + active project commitments + pipeline-coverage signals. Most operators don't have this view; it's a small revolution for a one-person consultancy. Not v1 — flagged here so the designer doesn't accidentally close the door on the slot.
