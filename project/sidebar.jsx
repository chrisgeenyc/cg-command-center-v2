// Sidebar — dark, lime active, user tile bottom; filters nav by persona
const { Icon } = window;

// Persona-gated visibility. Operator sees everything; Collaborator sees a subset.
const COLLAB_ALLOWED = new Set(["today", "projects", "pulse", "notes", "settings"]);

const Sidebar = ({ active, onNavigate, persona = "operator" }) => {
  const { NAV, NAV_SETTINGS } = window.CG_DATA;
  const isCollab = persona === "collab";

  const filterNav = (items) => isCollab ? items.filter((i) => COLLAB_ALLOWED.has(i.id)) : items;
  ...
  return (
    <aside className="sidebar" data-persona={persona} data-screen-label="Sidebar">
      <div className="brand">
