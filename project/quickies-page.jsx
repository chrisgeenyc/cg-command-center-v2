// Quickies — full triage page
const { Icon, PageHeader, MetricStrip } = window;

const CHANNEL_ICON = { email: "mail", linkedin: "linkedin", text: "text", call: "phone" };
const TINT_BG = {
  yellow: "#FFDE59",
...
function QuickieRow({ q, tints, onToggle }) {
  const bg = tints ? TINT_BG[q.tint] : "var(--surface)";
  return (
    <div className={`quickie ${q.done ? "done" : ""}`}
...
function RelationshipOverview() {
  const { RELATIONSHIP_OVERVIEW } = window.CG_DATA;
  return (
    <section className="rel-overview" data-screen-label="Relationship Overview">
      ...
      <div className="rel-overview-grid">
        ...
        {RELATIONSHIP_OVERVIEW.rows.map(r => (
          <React.Fragment key={r.id}>
            <div className="rel-cell rel-row-label">
              {r.label}
...
function QuickiesPage({ isMobile, tints }) {
  const { QUICKIES_FULL, QUICKIES_METRICS, QUICKIES_DONE_RECENT, QUICKIES_SUGGESTED } = window.CG_DATA;
  const [items, setItems] = React.useState(QUICKIES_FULL);
  ...
  return (
    <main className="main main-full" data-screen-label="Quickies">
      <PageHeader
        title={<>Quickies.</>}
...
window.QuickiesPage = QuickiesPage;
