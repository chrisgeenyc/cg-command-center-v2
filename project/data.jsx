// Static data — mirrors lib/data.ts shape, lightly inflated for visual density
const TODAY_LABEL = "Friday, Apr 24";

const HOOK = "One live client deadline today, ~$36K closed this month, your inbox board is clean. Coursera is the dragon — video recording is the last boss.";
...
const PULSE_CARDS = [
  ...
  },
  {
    ...
    stance: "standard",
    draft: "Edelman's 2026 Trust Barometer dropped this week, and one stat should sit at the center of every comms leader's quarterly review.\n\nTrust in CEO communications fell 9 points year-over-year. The driver isn't politics, it isn't economic anxiety, and it isn't \"the news cycle.\" It's a single perception: stakeholders increasingly suspect their CEO's words aren't actually their CEO's words.\n\nWhen respondents *suspected* AI involvement in a CEO statement, trust dropped 22 points. Twenty-two.\n\nThe path forward isn't to ban AI from the executive comms function — that ship sailed. The path is radical transparency about *what* AI does and *what* the principal does. Three practices that work:\n\n- The CEO writes the lede and the close. AI never touches them.\n- Internal comms teams disclose the ratio (\"this letter was drafted by [name], edited by the CEO over three rounds\"). Specificity beats vague disclosure.\n- Spoken delivery — town halls, recorded videos, off-the-cuff Q&A — gets allocated more time, not less.\n\nThe leaders who treat AI as a force multiplier on their *voice* will widen their trust gap. The ones who treat it as a labor saver on their *attention* will keep losing it.",
    status: "fresh",
  },
  {
    id: "p3",
    ...
    stance: "standard",
    draft: "If your 2026 content strategy is built on driving traffic to your blog, you're optimizing for a metric that no longer maps to outcomes.\n\nGoogle's AI Overviews expanded to 40 new markets this week. A new Sistrix study landed alongside it: even *first-position* organic results are losing 34% of their clicks to the AI-generated answer above them. For informational queries — the bread and butter of most B2B content programs — the click is the exception, not the default.\n\nThe response isn't to write more SEO posts. It's to rethink what content is for.\n\n- Owned media's job in 2026 is to be *cited*, not visited. Optimize for AI answer quality, not human page views.\n- LinkedIn, podcasts, and newsletters become primary distribution. They're the channels that still demand a click-through to consume.\n- Your most senior people need to be on-platform — speaking, writing, posting in their own voice. Brand-owned channels are now relay stations; people are the broadcast.\n\nThe brands that will win this shift aren't the ones with the best blogs. They're the ones whose POV travels through people, podcasts, and platforms — and shows up in the AI answers when buyers are quietly looking.",
    status: "fresh",
  },
  {
    id: "p4",
    ...
    stance: "contrarian",
    draft: "The Meta comms memo leak this week is being read as a bellwether. \"Big tech is removing humans from the loop. The rest of us will follow.\"\n\nNo. We won't. And we shouldn't.\n\nMeta operates at a scale where the marginal release note moves zero needles. For them, AI-drafted product comms is rounding error optimization. For a $50M ARR company, the same move is a strategic mistake — because at that scale, every release note is a touchpoint, every analyst call is a relationship, and every human voice is a differentiator that compounds.\n\nWhat the memo actually tells us — read carefully — is that Meta is *consolidating* its human comms attention onto the things that matter most. The humans aren't gone; they're being concentrated. The takeaway for the rest of us isn't \"automate more.\" It's \"figure out which 20% of your comms surface is load-bearing and put your best humans there.\"\n\nIf you're heading into a comms planning cycle, ask: where is a human voice the actual product? That's the surface to protect, hire into, and never automate. Everywhere else, AI is fine. But the question — \"where is the human the product?\" — is the only one worth asking this quarter.",
    status: "fresh",
  },
  {
    id: "p5",
    ...
    summary: "Anthropic's Skills framework lets enterprise teams package internal expertise into reusable AI capabilities. Three early adopters in the comms space — Edelman, Weber Shandwick, and a Fortune 50 in-house team — report 3x speedups on routine deliverables. Worth noting before the framework gets generic.",
    stance: "standard",
    draft: "Anthropic shipped a framework this week called Skills, and if you lead a comms function, it's the most directly applicable AI release of the quarter.\n\nThe pitch: package the proprietary, hard-to-replicate expertise inside your team — your house style guide, your crisis playbook, your messaging framework — into reusable AI capabilities. The model now writes *your* way, not its way.\n\nWhy this matters more than another 100B-parameter model launch:\n\n- The bottleneck for AI in comms has never been raw capability. It's been *fidelity to the brand voice*. Skills is the first deployment-ready answer.\n- The 3x time savings reported by early adopters (Edelman, Weber Shandwick, an unnamed F50 in-house team) come from one place: the AI now needs less editing because it starts from your guardrails, not OpenAI's defaults.\n- It runs on infrastructure your IT team already runs. No model retraining. No vendor lock-in conversation.\n\nThe move this week, if you're a comms lead: pick the *one* deliverable your team produces most often — weekly newsletter, exec brief, crisis statement template — and package its style + structure as a Skill. You'll have something working by Friday.\n\nThe firms that get good at packaging expertise as reusable AI Skills will compound a quarter-by-quarter advantage. The ones that wait for the perfect platform will miss the window.",
    status: "fresh",
  },
];

const PULSE_DRAFTS_SAVED = [
...
// Monthly revenue by channel (last 12 mo, rough K)
const PIPELINE_REVENUE_SERIES = {
...
// Projected revenue by service type (K)
const PIPELINE_FORECAST_BY_SERVICE = [
...
// HubSpot Lead Nurture (Lead/MQL lifecycle — not Quickies)
const LEAD_NURTURE = [
...
// Per-stage relationship grid (for Quickies overview)
...
// Pulse — saved cue manicure data
const PULSE_SAVED_CUES = [
  {
    id: "sc1",
...
const QUICKIES_METRICS = [
  ...
  { label: "Gone Silent",    value: "5",  sub: "contacts 90+ days untouched",        subDot: "risk",    trend: { dir: "up",   text: "+1" } },
];
...
// Sharisse's Today variant
const SHARISSE_HOOK = "Chris has Coursera due today and three deals stuck. Two things you can do that move the needle without asking him.";

const SHARISSE_PRIORITY = {
...
const SHARISSE_QUICKIES_SUGGESTED = [
...
window.CG_DATA = {
  TODAY_LABEL, HOOK, PRIORITY, METRICS, PROJECTS, BLOCKED,
  CALENDAR, WINS, QUICKIES, TASKS, NAV, NAV_SETTINGS,
  PIPELINE_METRICS, STUCK_DEALS, PIPELINE_DEALS, PIPELINE_LOSSES,
  PROJECTS_METRICS, PROJECT_CARDS,
  QUICKIES_METRICS, QUICKIES_FULL, QUICKIES_DONE_RECENT, QUICKIES_SUGGESTED, RELATIONSHIP_OVERVIEW,
  NOTES, CONNECTORS,
  PULSE_METRICS, PULSE_CARDS, PULSE_DRAFTS_SAVED, PULSE_ARCHIVE_COUNT, PULSE_SAVED_CUES, PULSE_QUEUE,
  PIPELINE_REVENUE_SERIES, PIPELINE_FORECAST_BY_SERVICE, LEAD_NURTURE,
  TEAM_MEMBERS,
  SHARISSE_HOOK, SHARISSE_PRIORITY, SHARISSE_METRICS, SHARISSE_WAITING, SHARISSE_QUICKIES_SUGGESTED,
};
