// Today — operator default + collaborator (Sharisse) variant
const { UtilityRow, Header, PriorityCard, Metrics, StatusList,
        Calendar, Wins, RightRail, CollabRail, MobileQuickieRow, MobileTaskRow,
        TodayPulseModule, Icon } = window;

// ─── COLLABORATOR VARIANT ─────────────────────────────────────────────────
function CollabHeader() {
  const { SHARISSE_HOOK } = window.CG_DATA;
  return (
...
function CollabPriority({ style }) {
  const { SHARISSE_PRIORITY } = window.CG_DATA;
  return (
    <section className={`priority collab-priority ${style}`} data-screen-label="Collab Priority">
...
function CollabMetrics() {
  const { SHARISSE_METRICS } = window.CG_DATA;
  return (
...
function WaitingOnChris() {
  const { SHARISSE_WAITING } = window.CG_DATA;
  return (
...
// ─── TODAY PAGE ───────────────────────────────────────────────────────────
function TodayPage({ tweaks, setTweak, isMobile, navigate }) {
  const isCollab = tweaks.persona === "collab";

  if (isCollab) {
    return (
      <React.Fragment>
        <main className="main" data-persona="collab" data-screen-label="Today (Collaborator)">
          {!isMobile && <UtilityRow onTheme={() => setTweak("theme", tweaks.theme === "dark" ? "light" : "dark")} theme={tweaks.theme} />}
          <CollabHeader />
          ...
          {/* Sharisse sees a slim "tasks assigned to me" list, not Chris's full project status */}
          <StatusList
  ...
  return (
    <React.Fragment>
      <main className="main" data-screen-label="Today">
        {!isMobile && <UtilityRow onTheme={() => setTweak("theme", tweaks.theme === "dark" ? "light" : "dark")} theme={tweaks.theme} />}
        <Header />
