...
import { CONNECTORS, TEAM_MEMBERS } from '@/lib/data';
      ...
      <Field label="What collaborators can see" help="Toggle visibility of specific Today modules in the collaborator view.">
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {[
            ...
            <label key={k as string} style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
              <Toggle on={v as boolean} />
              <span style={{ fontSize: 13, color: "var(--ink-2)" }}>{k as string}</span>
            </label>
          ))}
        </div>
      </Field>
      ...
      <Field label="Theme">
        <div className="checks">
          {["light", "dark", "auto"].map(o => (
            <button key={o} className={`chip ${theme === o ? "active" : ""}`} onClick={() => setTheme(o)}>{o}</button>
          ))}
        </div>
      </Field>
      <Field label="Density" help="Comfortable (default) keeps generous whitespace. Compact tightens row heights ~20%.">
        <div className="checks">
          {["comfortable", "compact"].map(o => (
            <button key={o} className={`chip ${density === o ? "active" : ""}`} onClick={() => setDensity(o)}>{o}</button>
          ))}
        </div>
      </Field>
...
const CONNECTOR_LETTER: Record<string, string> = { HubSpot: "Hs", Asana: "As", Outlook: "Ot", Slack: "Sl", LinkedIn: "Li", Zapier: "Zp" };
